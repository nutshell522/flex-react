      ...
      <nav className={styles['nav']}>
        <div className={styles['left']}>
          <Link to="/" className={styles['logo-wrapper']}>
            <img src="/LOGO/FlexLogoDark.png" alt="" className={`${styles['logo']}`} />
            <h1>FLEX</h1>
          </Link>
        </div>
        ...
        <div className={styles['right']}>
          <div className={styles['search-wrapper']}>
            <input type="search" className={styles['search-bar']} placeholder="搜尋" />
            <SearchIcon className={styles['search-icon']} />
          </div>
          <Link to="#" onClick={(e) => e.preventDefault()} className={`${styles['nav-bar-btn']}`}>
        ...
        <div
          ...
          <div className={styles['nav-detail-area']}>
            {item.middleCategories.map((middlecategory) => (
              <div key={middlecategory.id} className={styles['nav-detail-block']}>
                <Link to={`/w/${item.code}-${item.id}-${middlecategory.id}`}>
                  <h3 className={styles['nav-detail-item-title']}>{middlecategory.name}</h3>
                </Link>
                <ul className={styles['nav-detail-item-ul']}>
                  {middlecategory.bottomCategories.map((bottomcategory) => (
                    <li key={bottomcategory.id} className={styles['nav-detail-item-li']}>
                      <Link
